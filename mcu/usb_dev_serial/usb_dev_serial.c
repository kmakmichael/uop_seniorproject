...
// from https://github.com/Junaidk11/Adafruit_GPS_Module
#include "gps.h"
#include "gps_buff.h"
...
//
//*****************************************************************************
#define USB_GPIO_BASE               GPIO_PORTA_BASE
#define USB_GPIO_PERIPH             SYSCTL_PERIPH_GPIOA
#define USB_TX_GPIO_PIN             GPIO_PIN_1
#define USB_RX_GPIO_PIN             GPIO_PIN_0
...
Timer0IntHandler(void)
{
    ...
    char buf[31];
    char* nofix = "0:0,0\r\n";
    //if (hgps.fix ==1 || hgps.fix == 2) {
    snprintf(buf, 32, "%d:%0.9f,%0.9f\r\n", hgps.fix, hgps.latitude, hgps.longitude);
    int32_t i = 0;
    if (hgps.fix == 1 || hgps.fix == 2) {
        while(i < 31) {
            USBBufferWrite(&g_sTxBuffer, &buf[i], 1);
            ++i;
        }
    } else {
        USBBufferWrite(&g_sTxBuffer, nofix, 7);
    }
 }

// code from https://github.com/Junaidk11/Adafruit_GPS_Module:
...
main(void)
{
    ...
    // GPS stuff
    UART_Init();
    char residual = UARTCharGet(UART2_BASE);
    ...
    ROM_GPIOPinTypeUART(USB_GPIO_BASE, USB_TX_GPIO_PIN | USB_RX_GPIO_PIN);
    ...
    while(1)
    {
        ...
        if (buff_get_full(&hgps_buff)) {    /* Check if anything in buffer now */
            while (buff_read(&hgps_buff, &rx, 1)) {
                ...
                // USBBufferWrite(&g_sTxBuffer, &rx, 1);
            }
        }
        //
