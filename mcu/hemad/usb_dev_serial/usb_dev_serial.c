...
void USBTxFnThing() {
    ++calls;
    uint8_t* buf;
    USBBufferRead(&g_sRxBuffer, &buf, 3);
    if (buf[0] == 'G')
        next_state = 1;
    else if (buf[0] == 'S')
        next_state = 0;
...
main(void)
    ...
    while(1)
    {
        if (!moving && current_state != next_state) {
